...
class Base():
    ...
    def find_element(self, locator):
        t = WebDriverWait(self.driver, self.timeout, self.t).until(lambda x: x.find_element(*locator))
        return t
    ...
    def is_exists(self, locator):
        try:
            t = WebDriverWait(self.driver, self.timeout, self.t).until(lambda x: x.find_element(*locator).is_displayed())
            return t
        except:
            return False

    def is_not_exists(self, locator):
        t = WebDriverWait(self.driver, self.timeout, self.t).until_not(lambda x: x.find_element(*locator).is_displayed())
        return t

    def title_is(self, loctor):
        '''判断页面title'''
        try:
            result = WebDriverWait(self.driver, self.timeout, self.t).until(EC.title_is(loctor))
            return result
        except:
            return False
